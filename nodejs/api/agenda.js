		...
		for (var key in result) {
			...
			inOutStock(obj_uniq, fk_typeMove, res, function(err, obj, callback) {
				...
				doc.save(function(err, doc) {
					...
					// Astreinte
					/*if (fk_typeMove.values[doc.typeMove].add) {
					 delete obj._id; //for create
					 var add2 = new StockModel(obj);
					 
					 // Change the barCode
					 var arr_code = add2.barCode.split('#');
					 add2.barCode = arr_code[0] + '#' + fk_typeMove.values[doc.typeMove].add;
					 
					 
					 add2.typeMove = fk_typeMove.values[doc.typeMove].add;
					 
					 add2.save(function(err) {
					 if (err) {
					 console.log(err);
					 return;
					 }
