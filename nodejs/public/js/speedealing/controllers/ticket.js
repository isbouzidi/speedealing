...
angular.module('mean.system').controller('TicketController', ['$scope', '$routeParams', '$location', '$route', '$timeout', 'Global', 'pageTitle', '$http', 'socket', '$modal', '$upload', 'Ticket', function ($scope, $routeParams, $location, $route, $timeout, Global, pageTitle, $http, socket, $modal, $upload, Ticket) {
        $scope.global = Global;
        $scope.item = [];
...
//MyCtrl.$inject = ['pageTitleSetter'];
        ...
        $scope.addNew = function () {
            var modalInstance = $modal.open({
                templateUrl: '/partials/ticket/create.html',
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
                {field: 'name', displayName: 'Nom alerte', cellTemplate: '<div class="ngCellText"><a ng-href="#!/ticket/{{row.getProperty(\'_id\')}}" data-tooltip-options=\'{"position":"right"}\'> {{row.getProperty(col.field)}}</a></div>'},
            ...
            for (var i = 0; i < $files.length; i++) {
                ...
                if ($scope.ticket)
                    $scope.upload = $upload.upload({
                        url: 'api/ticket/file/' + $scope.ticket._id,
                        method: 'POST',
        ...
        $scope.suppressFile = function (id, fileName, idx) {
            $http({method: 'DELETE', url: 'api/ticket/file/' + id + '/' + fileName
            }).
                    success(function (data, status) {
                        if (status == 200) {
