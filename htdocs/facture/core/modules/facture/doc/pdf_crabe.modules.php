...
// Defini position des colonnes
    ...
    function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0) {
        ...
        if ($conf->facture->dir_output) {
            ...
            $deja_regle = 0; // $object->getSommePaiement();
            $amount_credit_notes_included = 0; //$object->getSumCreditNotesUsed();
            $amount_deposits_included = 0; //$object->getSumDepositsUsed();
            ...
            if (file_exists($dir)) {
                ...
                if (!empty($mysoc->cgv)) {
                    ...
                    $pdf->MultiCell(100, 4, $title, '', 'L');

                    $pdf->SetFont('', '', $default_font_size - 4);
                    $pdf->writeHTMLCell(190, 3, $this->posxdesc - 1, $tab_top, dol_htmlentitiesbr($mysoc->cgv), 0, 1);
                    $nexY = $pdf->GetY();
                    $height_cgv = $nexY - $tab_top;
                    ...
                    $tab_height = $tab_height - $height_cgv;
                }
    ...
    function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs) {
        ...
        if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT)) {
            ...
            if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_ISNULL) && $tvaisnull) {
                ...
                if ($object->type == 2)
                    $text = $outputlangs->transnoentities("TotalTTCToYourCredit");
    ...
    function _pagehead(&$pdf, $object, $showaddress, $outputlangs) {
        ...
        $objectidnext = false; //$object->getIdReplacingInvoice('validated');
        ...
        if ($object->type == "INVOICE_STANDARD") {
            $posy+=3;
            $pdf->SetXY($posx, $posy);
            $pdf->SetTextColor(0, 0, 60);
