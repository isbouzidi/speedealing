...
if ($action == 'specimen') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir . "commande/core/modules/commande/doc/pdf_" . $modele . ".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
...
foreach ($dirmodels as $reldir) {
	$dir = dol_buildpath($reldir . "commande/core/modules/commande/");
		...
		if (is_resource($handle)) {
			...
			while (($file = readdir($handle)) !== false) {
				if (substr($file, 0, 13) == 'mod_commande_' && substr($file, dol_strlen($file) - 3, 3) == 'php') {
					...
					require_once DOL_DOCUMENT_ROOT . "/commande/core/modules/commande/" . $file . '.php';
