...
class Societe extends nosqlDocument {
	...
	function thirdparty_and_contact_email_array($addthirdparty = 0) {
		...
		if ($this->values->email && $addthirdparty) {
			$this->values->name;
			// TODO: Tester si email non deja present dans tableau contact
			$contact_emails['thirdparty'] = $langs->trans("ThirdParty") . ': ' . dol_trunc($this->values->name, 16) . " &lt;" . $this->values->email . "&gt;";
		}
	...
	function thirdparty_and_contact_phone_array() {
		...
		if ($this->values->tel) {
			...
			$contact_phone['thirdparty'] = $langs->trans("ThirdParty") . ': ' . dol_trunc($this->values->name, 16) . " &lt;" . $this->values->tel . "&gt;";
		}
	...
	function get_codeclient($objsoc = 0, $type = 0) {
		...
		if ($conf->global->SOCIETE_CODECLIENT_ADDON) {
			...
			dol_syslog(get_class($this) . "::get_codeclient code_client=" . $this->values->code_client . " module=" . $var);
		}
	}
	...
	function get_codefournisseur($objsoc = 0, $type = 1) {
		...
		if ($conf->global->SOCIETE_CODEFOURNISSEUR_ADDON) {
			...
			dol_syslog(get_class($this) . "::get_codefournisseur code_fournisseur=" . $this->values->code_fournisseur . " module=" . $var);
		}
	}
	...
	function codeclient_modifiable() {
		...
		if ($conf->global->SOCIETE_CODECLIENT_ADDON) {
			...
			dol_syslog(get_class($this) . "::codeclient_modifiable code_client=" . $this->values->code_client . " module=" . $var);
			if ($mod->code_modifiable_null && !$this->values->code_client)
				return 1;
			if ($mod->code_modifiable_invalide && $this->values->check_codeclient() < 0)
				return 1;
	...
	function codefournisseur_modifiable() {
		...
		if ($conf->global->SOCIETE_CODEFOURNISSEUR_ADDON) {
			...
			dol_syslog(get_class($this) . "::codefournisseur_modifiable code_founisseur=" . $this->values->code_fournisseur . " module=" . $var);
			if ($mod->code_modifiable_null && !$this->values->code_fournisseur)
				return 1;
	...
	function check_codeclient() {
		...
		if ($conf->global->SOCIETE_CODECLIENT_ADDON) {
			...
			dol_syslog(get_class($this) . "::check_codeclient code_client=" . $this->values->code_client . " module=" . $var);
			$result = $mod->verif($this->db, $this->values->code_client, $this, 0);
			return $result;
		}
		else {
			return 0;
	...
	function check_codefournisseur() {
		...
		if ($conf->global->SOCIETE_CODEFOURNISSEUR_ADDON) {
			...
			dol_syslog(get_class($this) . "::check_codefournisseur code_fournisseur=" . $this->values->code_fournisseur . " module=" . $var);
			$result = $mod->verif($this->db, $this->values->code_fournisseur, $this, 1);
			return $result;
		}
		else {
			return 0;
	...
	function get_codecompta($type) {
		...
		if ($conf->global->SOCIETE_CODECOMPTA_ADDON) {
			...
			if ($type == 'customer')
				$this->values->code_compta = $mod->code;
			if ($type == 'supplier')
				$this->values->code_compta_fournisseur = $mod->code;

			return $result;
		}
		else {
			if ($type == 'customer')
				$this->values->code_compta = '';
			if ($type == 'supplier')
				$this->values->code_compta_fournisseur = '';
