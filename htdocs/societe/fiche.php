...
if (empty($reshook)) {
	...
	if ($action == 'confirm_delete' && $confirm == 'yes' && $user->rights->societe->supprimer) {
		...
		if ($result > 0) {
			header("Location: " . DOL_URL_ROOT . "/societe/list.php?idmenu=menu:thirdpartyenable");
			exit;
		} else {
			$langs->load("errors");
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if ($action == 'create') {
		...
		// Prospect/Customer
		print '<tr><td width="25%"><span class="fieldrequired"></span></td><td width="25%"></td>';
		...
		if (isset($_POST['fournisseur']))
			$object->fournisseur = GETPOST('fournisseur');
		print $object->select_fk_extrafields('fournisseur', "fournisseur");
		print '</td>';
		...
		// Supplier code
		//if (!empty($conf->fournisseur->enabled) && $object->fournisseur && !empty($user->rights->fournisseur->lire)) {
		print '<tr><td>' . $form->editfieldkey("Fournisseur", 'fournisseur', $object->fournisseur, $object, $user->rights->societe->creer, "select") . '</td>';
		print '<td>';
		print $form->editfieldval("Fournisseur", 'fournisseur', $object->fournisseur, $object, $user->rights->societe->creer, "select");
