...
;(function($, window, document)
{
	...
	doc.on('click', '.navigable li, .navigable li > span, .navigable li > a, .navigable li > b', function(event)
	{
		...
		if (back.length === 0)
		{
			...
			back = $('<div class="back"><span class="back-arrow"></span><span class="back-text">'+text+'</span></div>').prependTo(root).click(function(event)
			{
				var current = root.data('navigableCurrent'),
					target, right, backHeight, parentLi, parentLink;
				...
				mainUL.stop(true)[animate ? 'animate' : 'css']({ right: right+'%' });
		...
		if (load.length > 0)
		{
			...
			mainUL.stop(true)[animate ? 'animate' : 'css']({ right: -(current.parentsUntil('.navigable', 'ul').length*100)+'%' }, 'fast');
			load.stop(true).removeData('navigable-target');
			if (animate)
			{
				load.animate({ left: '-10%' }, 'fast', function()
				{
					load.remove();
					clicked.click();
				});
		...
		else if (clicked.hasClass('navigable-ajax'))
		{
			...
			{
				// Load indicator
				load = $('<div class="load" style="left: -10%"></div>').appendTo(root);
				...
				mainUL.stop(true)[animate ? 'animate' : 'css']({ right: -((allUL.length-1)*100+10)+'%' });

				// Show load
				load[animate ? 'animate' : 'css']({ left: '0%' });
