			...
			if (settings.horizontal)
			{
				...
				createH = function()
				{
					...
					hscroller.on('mousedown', function(event)
					{
						...
						var mouseX = event.pageX,
							hscrollerRight = hscroller.parseCSSValue('right');
						...
						function watchMouse(event)
						{
							var availableSpace = hscrollbar.width()-hscroller.innerWidth(),
								hscrollerPos = Math.max(0, Math.min(availableSpace, hscrollerRight+(event.pageX-mouseX)));
				...
				refreshH = function()
				{
					...
					hscrollbar.stop(true)[(settings.animate && init) ? 'animate' : 'css']({
						...
						right: ((settings.usePadding ? element.parseCSSValue('padding-right')+settings.padding.left : settings.padding.right)+scrollH)+'px',
