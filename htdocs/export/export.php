...
$array_selected = dol_getcache("export_selected_fields");
...
if ($action == 'selectfield') {
    ...
    if ($field == 'all') {
        foreach ($fieldsarray as $key => $val) {
            ...
            dol_setcache("export_selected_fields",$array_selected);
        }
    }
    else {
        $warnings = array();
...
if ($step == 1 || !$datatoexport) {
    ...
    print_fiche_titre($langs->trans("Export"));
    ...
    if (count($objexport->array_export_code)) {
        foreach ($objexport->array_export_code as $key => $value) {
            ...
            if ($objexport->array_export_perms[$key]) {
                print '<a href="' . $_SERVER["PHP_SELF"] . '?step=2&datatoexport=' . $objexport->array_export_code[$key] . '">' . img_picto($langs->trans("NewExport"), 'filenew') . '</a>';
            } else {
...
if ($step == 2 && $datatoexport) {
    ...
    $head[$h][0] = $_SERVER["PHP_SELF"] . '?step=2&datatoexport=' . $datatoexport;
    $head[$h][1] = $langs->trans("Step") . " 2";
    $hselected = $h;
    $h++;
...
//    $this->array_export_alias[0]=$module->export_fields_alias[$r];
    ...
    if (count($array_selected)) {
        print '<a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?step=3&datatoexport=' . $datatoexport . '">' . $langs->trans("NextStep") . '</a>';
    } else {
...
if ($step == 3 && $datatoexport) {
    ...
    $head[$h][0] = $_SERVER["PHP_SELF"] . '?step=3&datatoexport=' . $datatoexport;
    ...
    foreach ($array_selected as $code => $value) {
        ...
        if ($value < count($array_selected))
            print '<a href="' . $_SERVER["PHP_SELF"] . '?step=3&datatoexport=' . $datatoexport . '&action=downfield&field=' . $code . '">' . img_down() . '</a>';
        if ($value > 1)
            print '<a href="' . $_SERVER["PHP_SELF"] . '?step=3&datatoexport=' . $datatoexport . '&action=upfield&field=' . $code . '">' . img_up() . '</a>';
        print '</td>';
    ...
    if (count($array_selected)) {
        print '<a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?step=4&datatoexport=' . $datatoexport . '">' . $langs->trans("NextStep") . '</a>';
    }
...
if ($step == 4 && $datatoexport) {
    ...
    $head[$h][0] = $_SERVER["PHP_SELF"] . '?step=4&datatoexport=' . $datatoexport;
