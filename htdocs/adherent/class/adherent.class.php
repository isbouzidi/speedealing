 ...
 */
class Adherent extends nosqlDocument {
	...
	function getCivilityLabel($nohtmlentities = 0) {
		...
		if ($nohtmlentities)
			return $langs->transnoentities("Civility" . $code) != "Civility" . $code ? $langs->transnoentities("Civility" . $code) : $code;
		else
	...
	function _load_ldap_info() {
		...
		if ($this->country_code && $conf->global->LDAP_MEMBER_FIELD_COUNTRY)
		...
		if ($this->phone_perso && $conf->global->LDAP_MEMBER_FIELD_PHONE_PERSO)
			$info[$conf->global->LDAP_MEMBER_FIELD_PHONE_PERSO] = $this->phone_perso;
		if ($this->phone_mobile && $conf->global->LDAP_MEMBER_FIELD_MOBILE)
		...
		if ($this->note && $conf->global->LDAP_MEMBER_FIELD_DESCRIPTION)
		...
		// Subscriptions
		if ($this->first_subscription_date && $conf->global->LDAP_FIELD_MEMBER_FIRSTSUBSCRIPTION_DATE)
		...
		if ($this->last_subscription_date && $conf->global->LDAP_FIELD_MEMBER_LASTSUBSCRIPTION_DATE)
			$info[$conf->global->LDAP_FIELD_MEMBER_LASTSUBSCRIPTION_DATE] = dol_print_date($this->last_subscription_date, 'dayhourldap');
		if (isset($this->last_subscription_amount) && $conf->global->LDAP_FIELD_MEMBER_LASTSUBSCRIPTION_AMOUNT)
