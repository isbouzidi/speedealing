...
//            $this->db->free($result);
				...
				if (is_array($input) && !empty($input)) {
					...
					if ($input['type'] == 'text') {
						...
						if (!empty($input['label']))
							$more.=$input['label'] . '</td><td valign="top" colspan="2" align="left" style="padding: 4px !important;">';
		...
		if ($htmlname != "none") {
			...
			if (!empty($conf->global->FACTURE_DEPOSITS_ARE_JUST_PAYMENTS)) {
				...
				if (!$filter || $filter == "fk_facture_source IS NULL OR (fk_facture_source IS NOT NULL AND description='(DEPOSIT)')")
					print $langs->trans("CompanyHasAbsoluteDiscount", price($amount), $langs->transnoentities("Currency" . $conf->currency)) . ': ';
				else
					print $langs->trans("CompanyHasCreditNote", price($amount), $langs->transnoentities("Currency" . $conf->currency)) . ': ';
			}
	...
	function select_date($set_time = '', $prefix = 're', $h = 0, $m = 0, $empty = 0, $form_name = "", $d = 1, $addnowbutton = 0, $nooutput = 0, $disabled = 0, $fullday = '') {
		...
		if (!$set_time && $empty == 0)
			$set_time = dol_now('tzuser');

		// Analysis of the pre-selection date
		if (preg_match('/^([0-9]+)\-([0-9]+)\-([0-9]+)T?([0-9]+)?:?([0-9]+)?/', $set_time, $reg)) {
		...
		if ($d) {
			...
			if (strval($set_time) != '' && $set_time != -1) {
				if ($h)
					$formated_date = dol_print_date($set_time, "%d-%m-%Y %H:%M");
				else
			...
			$retstring.='<input id="' . $prefix . '" name="' . $prefix . '" class="input-unstyled datepicker _gldp" type="text" value="' . $formated_date . '"';
			...
			if ($h)
				$retstring.='<script> $(function() {
							$("input#' . $prefix . '").datetimepicker({
								dateFormat: "dd-mm-yy",
								timeFormat: "HH:mm",
								showAnim: "slide"
								});
	...
	function showrefnav($object, $paramid, $morehtml = '', $shownav = 1, $fieldid = 'rowid', $fieldref = 'ref', $morehtmlref = '', $moreparam = '') {
		...
		if (is_string($object->_id)) {
			return $object->_id;
		} else if (is_array($object->_id)) {
			return (object) $object->_id;
		} else if (is_object($object->_id) && isset($object->_id->{'$id'})) {
	 ...
	 */
	function showphoto($modulepart, $object, $width = 180) {
		global $conf;
	...
	function select_dolgroups($selected = '', $htmlname = 'groupid', $show_empty = 0, $exclude = '', $disabled = 0, $include = '', $enableonly = '') {
		...
		if ($count) {
			...
			foreach ($result as $obj) {
				...
				$out.= '<option value="' . $obj->name . '"';
				if ($disableline)
					$out.= ' disabled="disabled"';
				if ((is_object($selected) && $selected->id == $obj->name) || (!is_object($selected) && $selected == $obj->name)) {
					$out.= ' selected="selected"';
				}
				$out.= '>';
