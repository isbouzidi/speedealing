...
function pdf_build_address($outputlangs, $sourcecompany, $targetcompany = '', $targetcontact = '', $usecontact = 0, $mode = 'source', $deliverycompany = '') {
    ...
    if ($mode == 'target') {
        if ($usecontact) {
            ...
            // Country
            if ($targetcontact->country_code && $targetcontact->country_code != $sourcecompany->pays_code)
            ...
            // Country
            if ($targetcompany->country_code && $targetcompany->country_code != $sourcecompany->pays_code)
...
function pdf_writelinedesc(&$pdf, $object, $i, $outputlangs, $w, $h, $posx, $posy, $hideref = 0, $hidedesc = 0, $issupplierline = 0) {
    ...
    if ($object->lines[$i]->product_type == 9 && (!empty($object->lines[$i]->special_code) || !empty($object->lines[$i]->fk_parent_line))) {
...
function pdf_getlinedesc($object, $i, $outputlangs, $hideref = 0, $hidedesc = 0, $issupplierline = 0) {
    ...
    if ($idprod) {
        ...
        if ($prodser->name) {
            ...
            if (empty($hideref)) {
                if ($issupplierline)
                    $ref_prodserv = $prodser->name . ' (' . $outputlangs->transnoentitiesnoconv("SupplierRef") . ' ' . $ref_supplier . ')';   // Show local ref and supplier ref
                else
                    $ref_prodserv = $prodser->name; // Show local ref only
...
function pdf_getlineupexcltax($object, $i, $outputlangs, $hidedetails = 0) {
    ...
    if (($object->lines[$i]->product_type == 9 && !empty($object->lines[$i]->special_code)) || !empty($object->lines[$i]->fk_parent_line)) {
        ...
        if (empty($hidedetails) || $hidedetails > 1)
            return price($sign * $object->lines[$i]->subprice);
    }
}
...
function pdf_getlineupwithtax($object, $i, $outputlangs, $hidedetails = 0) {
    ...
    if (($object->lines[$i]->product_type == 9 && !empty($object->lines[$i]->special_code)) || !empty($object->lines[$i]->fk_parent_line)) {
        ...
        if (empty($hidedetails) || $hidedetails > 1)
            return price(($object->lines[$i]->subprice) + ($object->lines[$i]->subprice) * ($object->lines[$i]->tva_tx) / 100);
    }
}
...
function pdf_getlinetotalwithtax($object, $i, $outputlangs, $hidedetails = 0) {
    ...
    if ($object->lines[$i]->special_code == 3) {
        ...
        if (($object->lines[$i]->product_type == 9 && !empty($object->lines[$i]->special_code)) || !empty($object->lines[$i]->fk_parent_line)) {
            ...
            if (empty($hidedetails) || $hidedetails > 1)
                return price(($object->lines[$i]->total_ht) + ($object->lines[$i]->total_ht) * ($object->lines[$i]->tva_tx) / 100);
        }
    }
}
...
function pdf_getTotalQty($object, $type, $outputlangs) {
    ...
    for ($i = 0; $i < $nblignes; $i++) {
        if ($object->lines[$i]->special_code != 3) {
            ...
            } elseif ($type == 9 && ( ($object->lines[$i]->product_type == 9 && !empty($object->lines[$i]->special_code)) || !empty($object->lines[$i]->fk_parent_line))) {
                $special_code = $object->lines[$i]->special_code;
                if (!empty($object->lines[$i]->fk_parent_line))
                    $special_code = $object->getSpecialCode($object->lines[$i]->fk_parent_line);
                // TODO add hook function
