...
function dol_print_address($address, $htmlid, $mode, $id, $gps = false) {
	...
	if ($address) {
		$rtr.= nl2br($address);
		$showgmap = $showomap = 0;
		if ($mode == 'thirdparty' && !empty($conf->google->enabled) && !empty($conf->global->GOOGLE_ENABLE_GMAPS))
			$showgmap = 1;
		...
		if ($mode == 'member' && !empty($conf->google->enabled) && !empty($conf->global->GOOGLE_ENABLE_GMAPS_MEMBERS))
			$showgmap = 1;
		if ($mode == 'thirdparty' && !empty($conf->openstreetmap->enabled) && !empty($conf->global->OPENSTREETMAP_ENABLE_MAPS))
			$showomap = 1;
...
function dol_print_graph($htmlid, $width, $height, $data, $showlegend = 0, $type = 'pie', $showpercent = 0, $url = '') {
	...
	if ($jsgraphlib == 'flot') {
		if ($type == 'pie') {
			...
			$outputserie = 0;
			foreach ($data['series'] as $serie) {
				if (empty($data['seriestype'][$i]) || $data['seriestype'][$i] == 'bar') {
					$i++;
					continue;
				};
				if ($outputserie > 0)
...
function img_action($alt, $numaction) {
	...
	if ($alt == 'default') {
		if ($numaction == -1)
			$alt = $langs->transnoentitiesnoconv('ChangeDoNotContact');
		if ($numaction == 0)
			$alt = $langs->transnoentitiesnoconv('ChangeNeverContacted');
		if ($numaction == 1)
			$alt = $langs->transnoentitiesnoconv('ChangeToContact');
		if ($numaction == 2)
			$alt = $langs->transnoentitiesnoconv('ChangeContactInProcess');
		if ($numaction == 3)
			$alt = $langs->transnoentitiesnoconv('ChangeContactDone');
	}
...
function img_allow($allow, $alt = 'default') {
	...
	if ($alt == 'default')
		$alt = $langs->trans('Active');

	if ($allow == 1)
		return img_picto($alt, 'tick.png');
...
function vatrate($rate, $addpercent = false, $info_bits = 0, $usestarfornpr = 0) {
	...
	if ($info_bits & 1)
		$ret.=' ' . ($usestarfornpr ? '*' : constant('MAIN_LABEL_MENTION_NPR'));
	return $ret;
}
...
function price($amount, $form = 0, $outlangs = '', $trunc = 1, $rounding = -1, $forcerounding = -1) {
	...
	if ($outlangs->trans("SeparatorDecimal") != "SeparatorDecimal")
		$dec = $outlangs->trans("SeparatorDecimal");
	if ($outlangs->trans("SeparatorThousand") != "SeparatorThousand")
		$thousand = $outlangs->trans("SeparatorThousand");
	if ($thousand == 'None')
		$thousand = '';
...
function get_localtax($tva, $local, $thirdparty_buyer = "", $thirdparty_seller = "") {
	...
	if (is_object($thirdparty_buyer)) {
		if ($code_country != $thirdparty_buyer->country_code)
			return 0;
		if ($local == 1 && !$thirdparty_buyer->localtax1_assuj)
			return 0;  // TODO Not sure this is good
		elseif ($local == 2 && !$thirdparty_buyer->localtax2_assuj)
			return 0; // TODO Not sure this is good
	}
...
function get_product_localtax_for_country($idprod, $local, $countrycode) {
	...
	if ($local == 1)
		return $product->localtax1_tx;
	elseif ($local == 2)
		return $product->localtax2_tx;
	...
	if ($thirdparty_seller->country_id == 'ES' || $thirdparty_seller->country_code == 'ES') {
		if ($local == 1) { //RE
			...
			if (is_numeric($thirdparty_buyer->localtax1_assuj) && !$thirdparty_buyer->localtax1_assuj)
				return 0;
			if (!is_numeric($thirdparty_buyer->localtax1_assuj) && $thirdparty_buyer->localtax1_assuj == 'localtax1off')
				return 0;
		}
		elseif ($local == 2) { //IRPF
		...
		if ($idprod)
			return get_product_localtax_for_country($idprod, $local, $thirdparty_seller->country_code);
		else
			return -1;
	}
	return 0;
...
function dol_textishtml($msg, $option = 0) {
	if ($option == 1) {
		...
		elseif (preg_match('/<br/i', $msg))
			return true;
		elseif (preg_match('/<span/i', $msg))
			return true;
		elseif (preg_match('/<div/i', $msg))
			return true;
		elseif (preg_match('/<li/i', $msg))
			return true;
		elseif (preg_match('/<table/i', $msg))
			return true;
		elseif (preg_match('/<font/i', $msg))
			return true;
		elseif (preg_match('/<strong/i', $msg))
			return true;
		elseif (preg_match('/<img/i', $msg))
			return true;
		elseif (preg_match('/<i>/i', $msg))
			return true;
		elseif (preg_match('/<b>/i', $msg))
			return true;
...
function make_substitutions($chaine, $substitutionarray) {
	...
	foreach ($substitutionarray as $key => $value) {
		if ($key == '__SIGNATURE__' && (!empty($conf->global->MAIL_DO_NOT_USE_SIGN)))
			$value = '';
