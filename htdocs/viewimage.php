 ...
 * 		\brief      Wrapper to show images into Speedealing screens
...
if ((isset($_GET["modulepart"]) && $_GET["modulepart"] == 'companylogo') && !defined("NOLOGIN"))
	define("NOLOGIN", '1');
...
if ($entity == '')
	$entity = 1; // For backward compatibility
...
if ($modulepart) {
	...
	// If modulepart=module				Allows any module to open a file if file is in directory called DOL_DATA_ROOT/modulepart
		...
		if (preg_match('/^([a-z]+)_user_temp$/i', $modulepart, $reg)) {
			if ($user->rights->$reg[1]->lire || $user->rights->$reg[1]->read)
				$accessallowed = 1;
			...
			if ($perm || $subperm) {
				if (($perm && !$subperm && $user->rights->$modulepart->$perm) || ($perm && $subperm && $user->rights->$modulepart->$perm->$subperm))
					$accessallowed = 1;
				$original_file = $conf->$modulepart->dir_output . '/' . $original_file;
			}
			else {
				if ($user->rights->$modulepart->lire || $user->rights->$modulepart->read)
					$accessallowed = 1;
				$original_file = $conf->$modulepart->dir_output . '/' . $original_file;
			}
		}
...
if ($modulepart == 'barcode') {
	...
	$image = $grid->findOne(
			array("metadata" => array("filename" => $original_file, "class" => $class, "id" => GETPOST('id')))
	);

	//error_log(print_r(array("metadata" => array("filename" => $original_file, "class" => $class, "id" => GETPOST('id'))), true));
